...
#import data----
raw_KAP <- read_csv(here("Data/KAP/KAP_csv.csv"))
...
KAP_new <- KAP_new |> 
  mutate(flock_size = Number_of_goats_in_herd + Number_of_sheep_in_herd,
   ...
   animal_ownership = case_when(
    ...
    Number_of_goats_in_herd != 0 & Number_of_sheep_in_herd != 0 ~ "Sheep and Goat"))
...
#Rearrange columns
KAP_new2 <- KAP_new |> 
  select(1:4,106,5:12,108,109,86,87, everything())
